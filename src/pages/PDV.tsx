...
const PDV = () => {
  ...
  useEffect(() => {
    if (barbershopId) {
      ...
      // Real-time updates
      const channel = supabase.channel('pdv-updates').on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'appointments'
      }, () => {
  ...
  const fetchBarbershopId = async () => {
    try {
      const {
        data
      } = await supabase.from('barbers').select('barbershop_id').eq('user_id', user?.id).single();
  ...
  const fetchTodayAppointments = async () => {
    ...
    try {
      ...
      const {
        ...
        `).eq('barbershop_id', barbershopId).gte('scheduled_at', today.toISOString()).lt('scheduled_at', tomorrow.toISOString()).order('scheduled_at', {
        ascending: true
      });
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const {
        ...
        count: 'exact',
        head: true
      }).eq('barbershop_id', barbershopId).gte('scheduled_at', today.toISOString()).lt('scheduled_at', tomorrow.toISOString());

      // Faturamento do mês
      const firstDay = new Date(today.getFullYear(), today.getMonth(), 1);
      const {
        data: payments
      } = await supabase.from('payments').select('amount').eq('barbershop_id', barbershopId).gte('created_at', firstDay.toISOString()).eq('status', 'completed');
      const revenue = payments?.reduce((sum, p) => sum + Number(p.amount), 0) || 0;

      // Clientes ativos (distintos no mês)
      const {
        data: appointments
      } = await supabase.from('appointments').select('client_id').eq('barbershop_id', barbershopId).gte('scheduled_at', firstDay.toISOString());
  ...
  const fetchBarberStatuses = async () => {
    ...
    try {
      ...
      const {
        data: barbers
      } = await supabase.from('barbers').select('id, name, is_available').eq('barbershop_id', barbershopId);
      if (!barbers) return;

      // Para cada barbeiro, buscar agendamentos
      const statuses = await Promise.all(barbers.map(async (barber: any) => {
        // Buscar appointment em andamento
        const {
          data: current
        } = await supabase.from('appointments').select('id, client_id').eq('barber_id', barber.id).eq('status', 'in_progress').maybeSingle();
        let currentClientName: string | undefined;
        if (current) {
          const {
            data: clientProfile
          } = await supabase.from('profiles').select('full_name').eq('id', current.client_id).maybeSingle();
        ...
        const {
          data: next
        } = await supabase.from('appointments').select('scheduled_at').eq('barber_id', barber.id).eq('status', 'scheduled').gte('scheduled_at', now.toISOString()).order('scheduled_at', {
          ascending: true
        }).limit(1).maybeSingle();
        return {
          id: barber.id,
          ...
          nextAppointment: next ? new Date(next.scheduled_at).toLocaleTimeString('pt-BR', {
            hour: '2-digit',
            minute: '2-digit'
          }) : undefined
        } as BarberStatus;
    ...
    try {
      ...
      const {
        data
          ...
          service_id,
          services (name)
        `).eq('barbershop_id', barbershopId).gte('scheduled_at', firstDay.toISOString());
      ...
      // Top 5
      const sorted = Object.entries(counts || {}).sort(([, a], [, b]) => (b as number) - (a as number)).slice(0, 5).map(([name, count]) => ({
        name,
  ...
  return <div className="min-h-screen bg-gradient-to-b from-background to-secondary/20">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-6">
          ...
          <Button variant="outline" size="sm" className="text-slate-900 font-medium text-base bg-slate-300 hover:bg-slate-200">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-1">
            ...
            <CardContent className="space-y-3">
              {barberStatuses.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                  <AlertCircle className="h-12 w-12 mx-auto mb-2 opacity-50" />
                  <p>Nenhum barbeiro cadastrado</p>
                </div> : barberStatuses.map(barber => <div key={barber.id} className="p-4 rounded-lg border bg-card">
                  ...
                  {barber.currentClient && <div className="text-sm text-muted-foreground mt-2">
                      <Scissors className="h-3 w-3 inline mr-1" />
                      Atendendo: {barber.currentClient}
                    </div>}
                  
                  {barber.nextAppointment && <div className="text-sm text-muted-foreground mt-1">
          ...
          <Card className="lg:col-span-2">
            ...
            <CardContent>
              <div className="space-y-3">
                {upcomingAppointments.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                    <AlertCircle className="h-12 w-12 mx-auto mb-2 opacity-50" />
                    <p>Nenhum agendamento próximo</p>
                  </div> : upcomingAppointments.map(apt => <div key={apt.id} className="p-4 rounded-lg border bg-card hover:bg-accent/5 transition-colors">
          ...
          <Card className="lg:col-span-1">
            ...
            <CardContent>
              {popularServices.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                  <AlertCircle className="h-12 w-12 mx-auto mb-2 opacity-50" />
                  <p>Nenhum serviço registrado</p>
                </div> : <div className="space-y-3">
                  {popularServices.map((service, index) => <div key={service.name} className="flex items-center justify-between p-3 rounded-lg border bg-card">
                      <div className="flex items-center gap-3">
        ...
        <Card className="mt-6">
          ...
          <CardContent>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
              {todayAppointments.map(apt => <div key={apt.id} className="p-3 rounded-lg border bg-card text-sm">
                  <div className="flex justify-between items-start mb-2">
